...
public interface AdvanceRepository extends JpaRepository<Advance, Long> {
    ...
    @Query(nativeQuery = true,
        value = "select * from orders.trip_request_advance_payment a " +
            ...
            "t.driver_id = a.driver_id and t.contractor_id = a.contractor_id) where t.trip_status_code = 'assigned' "+
            "and t.trip_type_code ='motor' and a.is_cancelled = false and a.read_at is null and a.sms_sent_at is null "+
    ...
    @Query(nativeQuery = true, value = "select (case when count(pc)> 0 then true else false end) " +
        ...
        "(pc.trip_id = :trip_id and pc.contractor_id = :contractor_id and pc.order_id = :order_id " +
        "and pc.driver_id = :driver_id and pc.trip_num = :trip_num and pc.is_cancelled = false)")
