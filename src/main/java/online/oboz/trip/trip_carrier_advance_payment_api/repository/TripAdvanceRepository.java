...
@Repository
public interface TripAdvanceRepository extends JpaRepository<TripAdvance, Long> {

    @Query(" select ap from TripAdvance ap " +
        " where  ap.tripId = :trip_id " +
        "   and ap.driverId = :driver_id " +
        "   and ap.contractorId = :contractor_id ")
    TripAdvance find(@Param("trip_id") Long tripId,
    ...
    @Query("select pc " +
        " from TripAdvance pc " +
        " where pc.id = :id ")
    Optional<TripAdvance> find(@Param("id") Long id);

    @Query("select pc " +
        " from TripAdvance pc " +
        " where pc.advanceUuid = :uuid ")
    Optional<TripAdvance> find(@Param("uuid") UUID uuid);

    @Query("select pc " +
        " from TripAdvance pc " +
        " where pc.uuidAdvanceApplicationFile is null " +
        "   or pc.uuidContractApplicationFile is null ")
    List<TripAdvance> findRequestsWithoutFiles();
    ...
    @Query(nativeQuery = true, value = "select pc.id, " +
        ...
        "where t.num = :tripNum ")
    TripAdvance find(@Param("tripNum") String tripNum);
    ...
    List<TripAdvance> findForNotification();
